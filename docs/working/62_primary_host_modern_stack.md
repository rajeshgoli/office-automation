...
| Status `sensors` object | Contains `mac_last_active`, `external_monitor`, `motion_detected`, `door_open`, `window_open`, `co2_ppm`. Web TypeScript still also tolerates `mac_active`; Android models the current fields it needs. | Preserve current keys. If Rust adds `mac_active`, keep `mac_last_active` too. |
