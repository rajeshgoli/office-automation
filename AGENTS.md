...
the PR body for each ticket the PR finishes; "Implements #N" does not close it. Reviewers and scouts don't claim. When you are retired, sm deletes your worktree if nothing would be lost; if something there must outlive you (a running server, results not yet pushed), run `sm worktree keep --reason "<why>"`.
 ...
 6. Once clean, `gh pr merge --squash --delete-branch --match-head-commit <reviewed sha>`. `main` requires signed commits so that only I can land changes. You're working as me with my gh creds, so if the merge is blocked only because the commits are unsigned, add `--admin`. For any other blocker, such as conflicts or failing checks, fix it or ask me. Then delete any local branches and worktrees you created.
